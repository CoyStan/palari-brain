// One durable spend meter for the active-brain Gemini writer/answer calls and
// OpenAI LongMemEval judge calls. This is intentionally derived from the
// mature, hash-pinned J4 meter without changing that historical file. Failed
// or uncertain attempts retain their complete reservation; successful
// attempts are charged from provider-reported usage.
...
async function verifyJ4TranscriptLedgerCoherence({
  ...
  for (const [attemptId, start] of Object.entries(ledger.starts)) {
    ...
    if (start.provider === 'gemini') {
      if (start.model !== geminiModel) {
        throw new J4LiveError(
          'EVIDENCE_COHERENCE',
          'Active Brain Gemini transcript uses an unexpected model.',
        )
      }
...
export async function createActiveBrainMeteredTransport({
  answerGeneration,
  capUsd,
  ...
  if (!nonEmptyString(geminiModel) ||
    !answerGeneration ||
    typeof answerGeneration !== 'object' ||
    Array.isArray(answerGeneration) ||
    !writerGeneration ||
    typeof writerGeneration !== 'object' ||
    Array.isArray(writerGeneration)) {
    throw new J4LiveError(
      'ACTIVE_CONFIGURATION_MISSING',
      'Active Brain transport requires its model and generation contracts.',
    )
  }
  ...
  async function call({
    ...
    const expectedGeminiUrl =
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(geminiModel)}:generateContent`
    if ((provider === 'gemini' && url !== expectedGeminiUrl) ||
  ...
  return {
    async callGemini({
      ...
      const expectedGeminiUrl =
        `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(geminiModel)}:generateContent`
      if (request.url !== expectedGeminiUrl ||
