---
id: BRN-0039
title: "Validate sparse confirmation on frozen health question"
stream: evaluation
level: 1
parent_id: 
root_id: BRN-0039
...
claimed_at:
target_branch: "main"
branch: "ticket/BRN-0039-validate-sparse-confirmation-on-frozen-health-question"
worktree: "/home/quetza/palari-brain-worktrees/BRN-0039-validate-sparse-confirmation-on-frozen-health-question"
allowed_paths:
  - "STATUS.md"
  - "coding-sessions/tickets/open/BRN-0039-*.md"
  - "coding-sessions/tickets/closed/BRN-0039-*.md"
  - "coding-sessions/reports/BRN-0039-*.md"
  - "coding-sessions/human-report/BRN-0039-*.md"
  - "coding-sessions/handoffs/BRN-0039-*.md"
forbidden_paths:
  - ".env"
  - ".env.*"
  - "*.key"
...
verification:
  - "provider-free BRN-0039 private adapter preflight"
  - "npm test"
  - "npm run quickstart"
  - "npm run ticket -- check BRN-0039"
created: 2026-08-09
updated: 2026-08-09
---

# BRN-0039 Validate sparse confirmation on frozen health question

## Goal

Run one fresh alpha diagnostic of the frozen health-device question through
the founder-accepted sparse confirmation v8 path. Determine whether the
reviewer can report only material numbered findings, revise from newly found
evidence, and reach a safe closure without repetitive per-memory output.

## Scope

- Identity: `palari-health-confirmation-v9-2026-08-09`.
...
- Pin the accepted BRN-0038 product at merge commit `8782bfb`, confirmation
...
- Reuse the frozen canonical source store and verify its SHA-256 before and
  after the run. Create a new private result namespace and never mutate or
  reinterpret the consumed v8 or earlier results.
...
- Audit the provisional/final answer, reference, dataset-marked evidence,
  smallest decisive facts, selected/materially used evidence, every sparse
  finding and its host-bound evidence ID, confirmation closure, duplicate
  suppression, bridge/read/frontier activity, durable writes, provider
  usage/cost, source immutability, and any failure stage.
- Record the outcome in `STATUS.md` and BRN-0039 reports as an alpha
  diagnostic, never a benchmark regrade.

## Budget Gate

- Opening private aggregate ledger: exactly `$37.21155714`.
- Expected measured fresh spend: approximately `$0.01-$0.03`.
- Conservative fresh reservation ceiling: `$0.70`.
- Proposed aggregate authorization ceiling: `$37.91155714`.
- These figures are a proposal, not provider authority. No credential or
  ...
  numeric aggregate ceiling `$37.91155714` after the clean preflight is
...
- No other question, population run, sealed U8 access, historical-score
  change, publication, retry, reroll, or cap top-up.
...
1. Provider-free preflight proves the v9 namespace is absent, the ledger equals
   the declared opening value, confirmation schema is v8, the selected ID is
   exactly `gpt4_31ff4165`, sealed U8 is excluded, source custody matches the
   frozen predecessor, and runtime settings match this ticket.
2. No provider call occurs before explicit founder approval of aggregate cap
   `$37.91155714`; the runner reserves `$0.70` before the answer stage and
...
3. One no-retry invocation either completes the answer and one official judge
   ...
   occurs.
4. The audit records each confirmation page, candidate numbers, sparse findings,
   host-bound evidence IDs, implicit non-material candidates, page completeness,
   lower-ranked tail, closure, and evidence recurrence.
...
- The single v9 identity has one preserved outcome, or remains unconsumed
  because preflight or budget authorization failed.
...
- Another invocation, question, or aggregate cap requires new founder
  direction and a new ticket.

### Final Review Gate

- This is a founder-requested R1 alpha diagnostic with no product change.
  Independent review is not required; exact numeric-cap approval remains
  mandatory before the live call.
...
- `npm run quickstart`
- `npm run ticket -- check BRN-0039`
- `npm run ticket -- scope-check --committed-plus-dirty --target main BRN-0039`
- `git diff --check main...HEAD`

## Stop Conditions

- Stop if the work needs a path outside `allowed_paths` or touches `forbidden_paths`.
- Stop before credentials or providers if the result namespace exists, main is
