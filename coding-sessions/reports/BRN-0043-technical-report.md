...
- `src/reranker-transformers.mjs`: shared exact-token quadratic scheduler,
  ...
  single-flight lifecycle, explicit generic 512-token ceiling, safe metrics,
  and explicit close.
...
- `tests/reranker-transformers.contract.test.mjs`: schedule, order,
  serialization, lifecycle, metrics isolation, and success/failure disposal.
...
- `node --test tests/reranker-transformers.contract.test.mjs tests/reranker-ettin.contract.test.mjs tests/ettin-native-bakeoff.contract.test.mjs tests/retrieval-answer.contract.test.mjs tests/answer-confirmation.contract.test.mjs`: PASS, 74/74.
- `npm test`: PASS, 90/90.
- `npm run quickstart`: PASS, 6/6.
- `npm run test:legacy`: PASS, 932 pass, 15 optional skip, 0 fail across 947.
...
- Microbatch shapes can cause tiny fp32 kernel drift. The implementation keeps
  every current input but native rank parity still needs the external audited
  runtime/cache profile before a paid continuation.
